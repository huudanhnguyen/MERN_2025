...
const DealDaily = () => {
    ...
    const fetchAndSetNewDeal = async () => {
        try {
            ...
            if (response?.data?.success && response.data.products.length > 0) {
                ...
                localStorage.setItem('dailyDeal', JSON.stringify({
                    product: newProduct,
    ...
    if (!dealProduct) {
        return <div className='w-full border p-4 rounded-md'>Loading Daily Deal...</div>;
    }
        ...
        <div className='w-full border p-4 rounded-md h-full flex flex-col justify-between'>
            <div>
                ...
                <div className='text-center pt-4'>
                    <img src={dealProduct.thumb} alt={dealProduct.title} className='w-full h-auto object-contain mb-4' />
                    <h4 className='font-semibold truncate mb-2'>{dealProduct.title.toUpperCase()}</h4>
                    <div className='flex justify-center mb-2'>
                        {renderRatingStars(dealProduct.totalRating)}
                    </div>
                    <p className='text-lg font-semibold text-gray-800 mb-4'>{formatPrice(dealProduct.price)}</p>
                </div>
            </div>
            <div>
                ...
                <button className='w-full bg-main text-white font-semibold py-3 rounded-md hover:bg-red-700 transition-colors uppercase'>
                    Options
                </button>
            </div>
        </div>
